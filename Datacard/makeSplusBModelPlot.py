...
if opt.doBands:
  if opt.loadToyYields != '':
    ...
    toyFiles = glob.glob("./SplusBModels_%s/toys/toy_*.root"%opt.ext)
    if len(toyFiles) == 0:
      print "     * [ERROR] No toys files of form ./SplusBModels_%s/toys/toy_*.root. Skipping bands"%opt.ext
      opt.doBands = False
    else:
	...
	if not vetoToy: df_bands.loc[len(df_bands)] = values
      ...
      if opt.saveToyYields:
        print "      * Saving toy yields to: SplusBModels_%s/toyYields_%s.pkl"%(opt.ext,opt.xvar.split(",")[0])
        with open("SplusBModels_%s/toyYields_%s.pkl"%(opt.ext,opt.xvar.split(",")[0]),"w") as fD: pickle.dump(df_bands,fD)
