...
from commonTools import cprint, color
...
def get_parser():
    ...
    parser.add_argument("-o", "--option",       help="tagged category option [comb, untagm2, tagm2, combm2, re or other categories]",               default="comb", type=str)
...
def main():
    ...
    if combineCards:
        cprint("[INFO] Combine cards for {} categories".format(opt), colorStr="green")
        comb_opt = ["comb", "untagm2", "tagm2", "m2", "re"]    
        for mass in mass_interp:
            ...
            for cat in category__.keys():
                ...
                if opt == "tagm2" and cat not in categoryTag["M2tag"]:
                    continue
                if opt == "combm2" and cat not in (categoryTag["M2tag"]+categoryTag["M2Untag"]):
                    continue
            ...
            cprint("---> merged card: {}".format(card_comb))
            os.system("combineCards.py {} > {} ".format(cards, card_comb))

    # calculate the limit
    exp_opt_str = "" if args.doObserved else "--expectSignal 1 -t -1"
    unb_opt_str = "" if args.unBlind else "--run=blind"
    if limit:
        cprint("[INFO] Calculate the limit for {} categories".format(opt), colorStr="green")
        queues = []
        logfiles = []
        for mass in mass_interp:
            card_comb = "./cards/datacard_{}_runII_{}_{}.txt".format(decayMode, opt, mass)
            ...
            command_ = f"combineTool.py {card_comb} -M AsymptoticLimits -n _{opt}_{mass} -m {mass} {exp_opt_str} {unb_opt_str} -v {verbose}"
            logfile_ = f"./logger/limit_{opt}_{mass}.txt"
