...
def get_parser():
    ...
    parser.add_argument("-o", "--option",       help="tagged category option [comb, nom1, untagm2, untagm1, tagm2, tagm1, re]", default="comb", type=str)
...
def main():
    ...
    if significance:
        ...
        for mass in mass_interp:
            ...
            if opt == "re":
                card_comb = "./cards/datacard_{}_runII_Resolved_{}.txt".format(decayMode, mass)
            queue.append("combine {} -M GenerateOnly -t -1 --expectSignal 1 --toysFrequentist --saveToys -m {} -n _{}_{} &> ./logger/toys_{}_{}.txt".format(card_comb, mass, opt, mass, opt, mass))
        print("---> Executing the following commands using {} cores".format(n))
        pprint(queue)
        submit(queue)
        ...
        for mass in mass_interp:
            ...
            queue.append("combine {} -M Significance {} --toysFrequentist --toysFile {} -m {} -n _{}_{} &> ./logger/significance_{}_{}.txt".format(card_comb, exp_opt_str, toy_file, mass, opt, mass, opt, mass))
            if doObserved:
                queue.append("combine {} -M Significance -m {} -n _{}_{}_obs &> ./logger/significance_obs_{}_{}.txt".format(card_comb, mass, opt, mass, opt, mass))
        print("---> Executing the following commands using {} cores".format(n))
        pprint(queue)
        submit(queue)
